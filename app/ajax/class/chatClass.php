  ...
  class chatClass
  {
    ...
    */
    public function processChatInput($p_chat_text, $p_watson_api_context)
    {
      ...
      if(isset($watson_reply_msg['watson_api_context']))
      {
         ...
         //Se ocupa el nombre lo sacamos de la sesion, la fecha, la hora, el tipo de servicio y el status

         //$CLIENTE ---- $DATE ----- $TIME ---- $TYPE_SERVICE ---- $STATUS
         ...
         {
            $i_date = empty($watson_current_context->i_date) ? null : trim($watson_current_context->i_date);//se queda
            $i_time = empty($watson_current_context->i_time) ? null : trim($watson_current_context->i_time);// se queda
            $i_service = empty($watson_current_context->i_service) ? null : trim($watson_current_context->i_service);//type_service
            ...
            //$i_name=$_SESSION['firstName'].$_SESSION['lastName'];
            $reservationObj = new Reservation();
            $return_array = $reservationObj->createReservation('CREATE', array('r_date' => $i_date, 'r_time' => $i_time, 'r_num_of_people'=> $i_service, 'r_person_name'=>$i_name, 'r_status'=>'CONFIRMED'));// 'r_city'=>$i_city,
         ...
         if(isset($watson_current_context->action) && strtoupper($watson_current_context->action) == 'ACTION_CANCEL_RESERVATION' && isset($watson_current_context->i_reservation_number))
         {
            ...
            if(isset($reservation_data['r_status']) && $reservation_data['r_status'] != 'CANCELLED')
            {
               ...
               if(isset($return_array['status']) && $return_array['status'] == 'S')
               {
                  ...
                  $return_array['message'] = 'Ya ha sido notificada la veterinaria de tu cancelacion <b>' . $reservation_data['id'] . '</b> is taken care.';
               }
               else
               {
                  $return_array['status'] = 'E';
                  $return_array['message'] = 'Lo siento, una falla en el sistema se presento mientra tratabamos de cancelar <b>' . $reservation_data['id'] . '</b>.';
               }
            }
            else
            {
               $return_array['status'] = 'E';
               $return_array['message'] = 'El numero de cita no ha sido encontrado en nuestra base de datos.';
            }
         ...
         if(isset($watson_current_context->action) && strtoupper($watson_current_context->action) == 'ACTION_RESCHEDULE_RESERVATION' && isset($watson_current_context->i_reservation_number))
         {
            ...
            if(isset($reservation_data['r_status']) && $reservation_data['r_status'] != 'CANCELLED')
            {
               ...
               {
                  $return_array['status'] = 'S';
                  $return_array['message'] = 'Tu reservación con numero:  <b>' . $reservation_data['id'] . '</b> ha sido actualizada a la fecha: ' . $i_date . ' y hora: ' . $i_time;
               }
               else
               {
                  $return_array['status'] = 'E';
    ...
    */
    ...
    private function getWatsonResponse($p_input_text, $p_watson_context)
    {
      ...
      $api_url = "https://gateway.watsonplatform.net/conversation/api/v1/workspaces/" . $watson_waorkspace_id . "/message?version=2019-02-28";
      ...
      curl_setopt($ch, CURLOPT_USERPWD, 'apikey'.':'.'H_EjeqcEeqrPzO6KzkgCu3WvWAJdYf_zblNJo8bGA3r6');
